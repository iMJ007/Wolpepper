...
public class UnsplashSearchActivity extends AppCompatActivity implements RecyclerViewDataUpdateRequester {
    ...
    private boolean parseJson(String json, boolean isFromHashMap) {
        try {
            if (json.substring(0, 1).equals("{") && new JSONObject(json).has("error")) {
                Toast.makeText(UnsplashSearchActivity.this, json, Toast.LENGTH_LONG).show();
                return false;
            }
            ...
            for (int i = 0; i < resultJsonArray.length(); i++) {
                ...
                paper.setInMuzeiList(UtilityMethods.isInMuzeiActiveList(UnsplashSearchActivity.this, imageId));
    ...
    private void loadPapersList() {
        ...
        URL url = NetworkUtils.buildSearchUrl(searchQuery.toLowerCase(), String.valueOf(pageNumber), APP_ID);
    ...
    private class LoadCategoryPaperList extends AsyncTask<URL, Void, URL> {
        ...
        protected void onPreExecute() {
            if (isFinishing() || isSearchResultsEmpty) {
                cancel(true);
                return;
            }
        ...
        protected URL doInBackground(URL... params) {
            URL url = params[0];
            try {
                String json = NetworkUtils.getResponseFromHttpUrl(url, UnsplashSearchActivity.this);
        ...
        protected void onPostExecute(final URL url) {
            ...
            if (isRateLimitReached) {
                UtilityMethods.showRateLimitReachedAlert(UnsplashSearchActivity.this);
                return;
            }

            if (url != null) {
                final Alerter alerter = Alerter.create(UnsplashSearchActivity.this);
                alerter.setTitle(getString(R.string.connection_error))
                        .setTextTypeface(ResourcesCompat.getFont(UnsplashSearchActivity.this, R.font.spacemono_regular))
                        .setTitleTypeface(ResourcesCompat.getFont(UnsplashSearchActivity.this, R.font.spacemono_bold))
            ...
            if (isSearchResultsEmpty) {
                Alerter.clearCurrent(UnsplashSearchActivity.this);
                boolean isMoreImageNotAvailable = pageNumber > 0;
                Alerter.create(UnsplashSearchActivity.this)
                        .setTextTypeface(ResourcesCompat.getFont(UnsplashSearchActivity.this, R.font.spacemono_regular))
                        .setTitleTypeface(ResourcesCompat.getFont(UnsplashSearchActivity.this, R.font.spacemono_bold))
                        .setTitle("Oops")
                        .setText(isMoreImageNotAvailable ? ("No more images found for '" + searchQuery + "'.") : ("No images found for '" + searchQuery + "'."))
                        .setBackgroundColorRes(R.color.alert_default_error_background)
                        .enableVibration(true)
                        .show();
                return;
